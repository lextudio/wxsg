...
internal static class RuntimeHelpersEmitter
{
    ...
    private static void EmitUnknownMarkupExtensionHelper(GraphEmitter emitter)
    {
        ...
        // FEF variant: constructs the MarkupExtension instance but does NOT call ProvideValue().
        // FrameworkElementFactory.SetValue() accepts a MarkupExtension and defers ProvideValue
        // to template-instantiation time, but rejects Visual/ContentElement results up-front.
        sb.AppendLine(i + "private static global::System.Windows.Markup.MarkupExtension __WXSG_CreateUnknownMarkupExtensionForFef(");
        sb.AppendLine(i + "    string __xmlNsUri,");
        sb.AppendLine(i + "    string __localName,");
        sb.AppendLine(i + "    string[] __positionalArgs,");
        sb.AppendLine(i + "    string[] __namedArgKeys,");
        ...
        sb.AppendLine(i + "            var __part = __parts[__p].Trim();");
        sb.AppendLine(i + "            if (__part.StartsWith(\"assembly=\", global::System.StringComparison.OrdinalIgnoreCase))");
        sb.AppendLine(i + "            { __assemblyName = __part.Substring(\"assembly=\".Length).Trim(); break; }");
        sb.AppendLine(i + "        }");
        sb.AppendLine(i + "        foreach (var __assembly in __assemblies)");
        sb.AppendLine(i + "        {");
        sb.AppendLine(i + "            if (!string.IsNullOrEmpty(__assemblyName) && !string.Equals(__assembly.GetName().Name, __assemblyName, global::System.StringComparison.OrdinalIgnoreCase))");
        ...
        sb.AppendLine(i + "                var __fullTypeName = string.IsNullOrEmpty(__clrNs) ? __typeName : __clrNs + \".\" + __typeName;");
        sb.AppendLine(i + "                var __type = __assembly.GetType(__fullTypeName, throwOnError: false);");
        sb.AppendLine(i + "                if (__type is null || !typeof(global::System.Windows.Markup.MarkupExtension).IsAssignableFrom(__type)) continue;");
        sb.AppendLine(i + "                object __instance;");
        sb.AppendLine(i + "                try");
        sb.AppendLine(i + "                {");
        sb.AppendLine(i + "                    __instance = (__resolvedArgs is null || __resolvedArgs.Length == 0)");
        ...
        sb.AppendLine(i + "                    }");
        sb.AppendLine(i + "                }");
        sb.AppendLine(i + "                return (global::System.Windows.Markup.MarkupExtension)__instance;");
        sb.AppendLine(i + "            }");
        sb.AppendLine(i + "        }");
        sb.AppendLine(i + "    }");
        sb.AppendLine(i + "    foreach (var __assembly in __assemblies)");
        sb.AppendLine(i + "    {");
        sb.AppendLine(i + "        var __attrs = __assembly.GetCustomAttributes(typeof(global::System.Windows.Markup.XmlnsDefinitionAttribute), inherit: false);");
        sb.AppendLine(i + "        foreach (global::System.Windows.Markup.XmlnsDefinitionAttribute __def in __attrs)");
        sb.AppendLine(i + "        {");
        sb.AppendLine(i + "            if (!string.Equals(__def.XmlNamespace, __xmlNsUri, global::System.StringComparison.Ordinal)) continue;");
        sb.AppendLine(i + "            var __clrNs = __def.ClrNamespace ?? string.Empty;");
        sb.AppendLine(i + "            foreach (var __typeName in __typeNames)");
        sb.AppendLine(i + "            {");
        sb.AppendLine(i + "                var __fullTypeName = string.IsNullOrEmpty(__clrNs) ? __typeName : __clrNs + \".\" + __typeName;");
