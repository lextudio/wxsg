...
public sealed class WpfXamlSourceGenerator : IIncrementalGenerator
{
    ...
    private static string BuildThemeLoaderSource(string assemblyName, ImmutableArray<string> targetPaths)
    {
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            __WxsgDiag(\"Initialize ENTER (module init for assembly)\");");
        sb.AppendLine("            var app = global::System.Windows.Application.Current;");
        sb.AppendLine("            if (app is null) { __WxsgDiag(\"Initialize: Application.Current is NULL, returning\"); return; }");
        sb.AppendLine("            __WxsgDiag(\"Initialize: Application.Current OK, Windows.Count=\" + app.Windows.Count);");
        sb.AppendLine("            try");
        sb.AppendLine("            {");
        sb.AppendLine("                // If the app already has windows open (late/lazy assembly load), merge now.");
        sb.AppendLine("                if (app.Windows.Count > 0)");
        sb.AppendLine("                {");
        sb.AppendLine("                    __WxsgDiag(\"Initialize: windows already open, calling MergeResources synchronously\");");
        ...
        sb.AppendLine("                app.Startup += (s, e) => { __WxsgDiag(\"Startup handler fired, calling MergeResources\"); MergeResources((global::System.Windows.Application)s); };");
        sb.AppendLine("                __WxsgDiag(\"Initialize: registered for Application.Startup\");");
        sb.AppendLine("                return;");
        sb.AppendLine("            }");
        sb.AppendLine("            catch (global::System.Exception __wxsg_ex) { __WxsgDiag(\"Initialize EXCEPTION: \" + __wxsg_ex.GetType().Name + \": \" + __wxsg_ex.Message); }");
        sb.AppendLine("            __WxsgDiag(\"Initialize: fallback MergeResources (no dispatcher path)\");");
        sb.AppendLine("            MergeResources(app);");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("            try");
        sb.AppendLine("            {");
        sb.AppendLine("                __WxsgDiag(\"MergeDict ENTER \" + uri);");
        ...
        sb.AppendLine("                __WxsgDiag(\"MergeDict loaded rd=\" + (__wxsg_rd != null) + \" count=\" + (__wxsg_rd != null ? __wxsg_rd.Count : -1));");
        ...
        sb.AppendLine("                    {");
        sb.AppendLine("                        try { if (__wxsg_existing?.Source != null && __wxsg_existing.Source.Equals(__wxsg_src)) { __WxsgDiag(\"MergeDict already merged, skipping\"); return; } } catch { }");
        sb.AppendLine("                    }");
        sb.AppendLine("                }");
        sb.AppendLine("                app.Resources.MergedDictionaries.Add(__wxsg_rd);");
        sb.AppendLine("                __WxsgDiag(\"MergeDict ADDED to app.Resources.MergedDictionaries (now \" + app.Resources.MergedDictionaries.Count + \" dicts)\");");
        sb.AppendLine("            }");
        sb.AppendLine("            catch (global::System.Exception __wxsg_ex) { __WxsgDiag(\"MergeDict EXCEPTION: \" + __wxsg_ex.GetType().Name + \": \" + __wxsg_ex.Message); }");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        private static void __WxsgDiag(string msg)");
        sb.AppendLine("        {");
        sb.AppendLine("            try");
        sb.AppendLine("            {");
        sb.AppendLine("                var path = global::System.IO.Path.Combine(global::System.IO.Path.GetTempPath(), \"AvalonDock_DragDebug.log\");");
        sb.AppendLine("                global::System.IO.File.AppendAllText(path, \"[\" + global::System.DateTime.Now.ToString(\"HH:mm:ss.fff\") + \"] WXSG: \" + msg + global::System.Environment.NewLine);");
        sb.AppendLine("            }");
        sb.AppendLine("            catch { }");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
