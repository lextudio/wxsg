...
public sealed class WpfXamlSourceGenerator : IIncrementalGenerator
{
    ...
    private static string BuildThemeLoaderSource(string assemblyName, ImmutableArray<string> targetPaths)
    {
        ...
        sb.AppendLine("            try");
        sb.AppendLine("            {");
        sb.AppendLine("                var dispatcher = app.Dispatcher;");
        sb.AppendLine("                if (dispatcher is not null)");
        sb.AppendLine("                {");
        sb.AppendLine("                    dispatcher.BeginInvoke(new global::System.Action(() => MergeResources(app)));" );
